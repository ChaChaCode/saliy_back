...
### Response (для `CARD_ONLINE` / `YANDEX_PAY` — оплата онлайн):
```json
{
  "id": "uuid",
...
👉 Для `CARD_ONLINE` / `YANDEX_PAY` клиент должен немедленно сделать `window.location.href = paymentUrl`, чтобы попасть на платёжную форму банка/Яндекса.
...
1. **Пользователь заполняет форму** с данными:
   ...
   - Адрес доставки
   - Тип доставки
   - **Метод оплаты** (на выбор): `CARD_ONLINE` (Альфа-Банк) / `YANDEX_PAY` (Яндекс Пэй) / `CARD_MANUAL` / `CRYPTO` / `PAYPAL`
