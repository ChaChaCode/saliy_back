...
Товар списывается со склада в момент создания заказа (резервируется под него). Если онлайн-заказ так и не оплатили, он не должен держать товар вечно. Поэтому фоновая задача раз в минуту находит онлайн-заказы в статусе `PENDING` старше заданного таймаута, отменяет их (статус `CANCELLED`) и возвращает товар на склад. Таймаут задаётся переменной `ORDER_PENDING_TIMEOUT_MIN` (по умолчанию 15 минут). Заказы, которые уже оплачены (`isPaid=true`) или не являются онлайн-оплатой, автоотмена не трогает.

Чтобы клиент не смог оплатить уже отменённый заказ, срок жизни платёжной ссылки Точки (`ttl`, в минутах) приравнивается к этому же таймауту. По истечении ссылка протухает (Точка переводит платёж в `EXPIRED`) ровно тогда, когда заказ автоматически отменяется. Срок Точки по умолчанию (без передачи `ttl`) — 7 дней, поэтому передавать его обязательно.
