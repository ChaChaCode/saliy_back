...
Безопасность подтверждения (важно): backend НЕ доверяет статусу из тела webhook (его можно подделать). Webhook от Точки/Яндекса — лишь сигнал «проверь заказ»; реальный статус оплаты backend переспрашивает у API платёжки (Точка — по operationId, Яндекс — по orderNumber) и только по ответу API помечает заказ оплаченным. Поэтому поддельный webhook не сможет пометить заказ оплаченным. Подтверждение идемпотентно: уже оплаченный заказ повторными webhook не обрабатывается дважды (чек/корзина/накладная не дублируются).
