...
- `BANK_TRANSFER` (оплата по реквизитам) — служебный способ ТОЛЬКО для админки (ручные заказы, оплата по реквизитам через менеджера). На витрине магазина в выборе не показывается.
