...
Онлайн-платёжки на выбор пользователю:

| `paymentMethod` | Через что | Способ |
|---|---|---|
| `CARD_ONLINE` | **Альфа-Банк** (Alfa RBS) | карта |
| `YANDEX_PAY` | **Яндекс Пэй** | карта / SPLIT / SBP |
| `SBP_TOCHKA` | **Банк Точка** (Tochka Pay) | СБП (QR / ссылка) |
| `CARD_MANUAL` / `CRYPTO` / `PAYPAL` | Ручная обработка менеджером | — |

Для онлайн-платежей (Альфа, Яндекс, Точка) заказ создаётся в `PENDING`, в ответе возвращается `paymentUrl` для редиректа клиента (для Точки это ссылка/QR СБП). Для остальных методов — `CONFIRMED` сразу.
