# API авторизации

Авторизация по email с 6-значным кодом подтверждения. Базовый адрес API: `https://saliyclothes.com/api`.

---

## Эндпоинты

### 1. Отправить код подтверждения

**POST** `/api/auth/send-code`

Отправляет 6-значный код подтверждения на email.

**Поля запроса:**

| Поле | Тип | Обязательное | Описание |
|------|-----|--------------|----------|
| email | строка | да | Email пользователя |

**Ответ:** сообщение об успешной отправке кода.

**Ошибки:**
- `400` — код уже был отправлен, повторная отправка доступна через некоторое время.
- `429` — превышен лимит запросов (не более 5 отправок в минуту).

---

### 2. Подтвердить код и авторизоваться

**POST** `/api/auth/verify-code`

Подтверждает код и авторизует пользователя.

**Поля запроса:**

| Поле | Тип | Обязательное | Описание |
|------|-----|--------------|----------|
| email | строка | да | Email пользователя |
| code | строка | да | 6-значный код из письма |

**Что происходит при успехе:**
- access token и refresh token устанавливаются в httpOnly cookie;
- access token дополнительно возвращается в теле ответа (для обратной совместимости со старым фронтом);
- к пользователю автоматически привязываются его прошлые гостевые заказы — все заказы без владельца, оформленные на эту же почту, получают его как владельца и появляются в «Моих заказах». Email на этом шаге уже подтверждён кодом, поэтому привязка безопасна.

**Устанавливаемые cookie:**

| Cookie | Срок жизни | Параметры |
|--------|-----------|-----------|
| accessToken | 15 минут | httpOnly, secure |
| refreshToken | 7 дней | httpOnly, secure |

**Ошибки:**
- `401` — неверный или истёкший код; либо превышено число попыток ввода (после 5 неверных попыток код инвалидируется — нужно запросить новый).
- `429` — превышен лимит запросов (не более 10 попыток в минуту).

---

### 3. Обновить access token

**POST** `/api/auth/refresh`

Обновляет access token, используя refresh token из cookie. Запрос должен отправляться с передачей cookie (для cross-origin фронта — режим включения учётных данных).

**Что происходит при успехе:**
- новые access и refresh token устанавливаются в httpOnly cookie;
- access token также возвращается в теле ответа.

**Ошибки:**
- `401` — refresh token не найден или недействителен.

---

### 4. Выйти из системы

**POST** `/api/auth/logout`

Отзывает refresh token и очищает обе cookie (accessToken и refreshToken).

**Ответ:** сообщение об успешном выходе.

---

## Токены

### Access token
- Время жизни: 15 минут.
- Хранится в httpOnly cookie `accessToken`.
- Принимается сервером из cookie `accessToken` либо из заголовка `Authorization` (формат Bearer) — заголовок поддерживается для обратной совместимости.
- Используется для всех защищённых эндпоинтов.

### Refresh token
- Время жизни: 7 дней.
- Хранится в httpOnly cookie `refreshToken`.
- Используется только для обновления access token. Одноразовый: при обновлении старый refresh инвалидируется и выдаётся новый (ротация).

### Политика SameSite для cookie
Атрибут SameSite у auth-cookie управляется переменной окружения `COOKIE_SAMESITE`:

| Значение | Когда использовать |
|----------|--------------------|
| lax (по умолчанию) | Фронт и API на одном сайте (saliyclothes.com и его поддомены). Строже и безопаснее. |
| none | Фронт на другом домене (cross-site, например Vercel). Браузер отправляет cookie на cross-site запросы; требует HTTPS (Secure ставится автоматически). |
| strict | Максимально строгий режим. |

Cookie ставятся без атрибута Domain — привязаны к хосту, который их выдал (API-домен). Для cross-site сценария фронт может также проксировать запросы через свой сервер (тогда cookie остаются first-party и работают в любом браузере, включая Safari).

---

## Сценарий авторизации

1. Запросить код подтверждения на email.
2. Получить код из письма.
3. Подтвердить код — сервер выставит httpOnly cookie с токенами.
4. Обращаться к защищённым эндпоинтам — браузер автоматически отправляет cookie (фронт должен включать передачу учётных данных при cross-origin запросах).
5. При истечении access token обновить его через refresh.
6. Выйти из системы при завершении сессии.

---

## Коды ошибок

| Код | Описание |
|-----|----------|
| 200 | Успешно |
| 400 | Некорректные данные |
| 401 | Не авторизован |
| 500 | Ошибка сервера |
