...
Накладная CDEK создаётся автоматически при подтверждении оплаты заказа с доставкой CDEK — после этого появляется `cdekUuid`, затем `cdekNumber` (трек-номер) и статусы доставки приходят через webhook CDEK (`POST /api/delivery/webhook`, регистрируется в кабинете CDEK). Статус «где посылка сейчас» виден в полях `cdekStatus` / `cdekStatusName`, трек-ссылка — в `cdekTrackingUrl`. Обновить статус вручную — `POST /api/admin/orders/:orderNumber/cdek/refresh`.
