...
Подписка на вебхуки CDEK регистрируется только через API CDEK (не в личном кабинете): запросом на адрес вебхуков CDEK с типом события «статус заказа» и нашим URL уведомлений. При повторной регистрации с тем же типом события CDEK заменяет старый URL новым.
