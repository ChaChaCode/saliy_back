...
- `excludeNewItems` - не применять к товарам с cardStatus=NEW (по умолчанию true, для FREE_DELIVERY игнорируется)
...
### 7. Скидка, применимая к новинкам
```json
{
  "code": "NEWITEMS15",
  "type": "PERCENTAGE",
...
6. **Новинки:** По умолчанию (`excludeNewItems=true`) промокоды не применяются к товарам с `cardStatus='NEW'`, кроме типа `FREE_DELIVERY`. Можно отключить эту проверку, установив `excludeNewItems=false`
