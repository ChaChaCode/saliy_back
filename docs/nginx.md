...
3. **HTTP/2.** Один мультиплексированный коннект вместо шести параллельных
   HTTP/1.1. В nginx 1.25.1+ включается директивой `http2 on;`, в 1.24 и
   ранее — суффиксом в listen: `listen 443 ssl http2;` (директива `http2 on;`
   на 1.24 даёт ошибку "unknown directive"). Проверять версию: `nginx -v`.
...
- **HTTP/2 «не включается» / "unknown directive http2".** Зависит от версии:
  1.25.1+ — `http2 on;`; 1.24 и раньше — суффикс `listen 443 ssl http2;`.
  Проверять `nginx -v`. curl без `--http2` покажет HTTP/1.1 даже когда h2
  работает — тестировать с флагом `--http2`.
- **`gzip`/`server_tokens`/`types_hash` duplicate.** Стоковый nginx.conf уже
  их содержит — в своём tuning-файле НЕ повторять `gzip on;` (только
  `gzip_types` и параметры).
- **Двойной Cache-Control на статике.** `expires 1y;` и `add_header
  Cache-Control` ставят заголовок каждый — использовать что-то одно
  (add_header с immutable полнее).
